...
class ImagensControllers {
  static async cadastraImagens(req, res) {
    var imgs = [];
  ...
  static async pegaimagemById(req, res) {
    const { id } = req.params;
    try {
      const mostraImagens = await database.Imagem.findOne({
        where: { id: Number(id) },
        attributes: [
          "id",
          "mimetype",
  ...
  static async pegaImagens(req, res) {
    try {
      const mostraImagens = await database.Imagem.findAll({
        order: ["id"],
        attributes: [
          "id",
          "mimetype",
...
module.exports = ImagensControllers;